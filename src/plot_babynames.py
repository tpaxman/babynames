...
chosen = ('Ezra Spencer Ewan Aaron Alex Alexander Archie Cameron Ian Colin Declan Edward ' +
          'Isaac Nicholas Quinn Samuel Thomas Timothy')
data = df[df.babyname.isin(chosen.split()) & df.sex.isin(['Male'])]
(alt.Chart(data).mark_line()
 .encode(x='year',
         y='fraction',
         facet=alt.Facet('babyname', columns=4),
         color='region')
